...
import { prompt, runMigrationRunnerCommand, runSingleTask, syncLocalDbToProd } from './tasks';
...
const promptCommand = () =>
  prompt<'deploy' | 'seed' | 'sql_dump' | 'sql_load' | 'sync'>({
    type: 'select',
    name: 'cmd',
    message: 'Select a command',
    choices: [
      { title: 'Sync local', description: 'Sync local DB with production', value: 'sync' },
...
async function main() {
  ...
  if (cmd === 'sync') {
    ...
    if (cmd === 'seed' || cmd === 'deploy') {
      return runMigrationRunnerCommand({ stack: stack, payload: { command: cmd } });
    } else if (cmd === 'sql_dump') {
      return runMigrationRunnerCommand({ stack: stack, payload: { command: 'sql_dump', dumpType: 'manual' } });
      ...
      return runMigrationRunnerCommand({ stack: stack, payload: { command: 'sql_load', dumpKey } });
    }
  }
}
